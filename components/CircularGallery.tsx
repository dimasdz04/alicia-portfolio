...
function autoBind(instance: unknown): void {
  ...
  Object.getOwnPropertyNames(proto).forEach((key) => {
    if (key !== 'constructor' && typeof target[key] === 'function') {
      ...
      target[key] = (target[key] as any).bind(target);
    }
  });
}
...
class App {
  ...
  startX: number = 0;
  startY: number = 0;
  startTime: number = 0; // Tambahan untuk mendeteksi lama klik

  constructor(
    container: HTMLElement,
    { items, bend = 1, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05, onClick }: AppConfig
  ) {
    ...
    this.originalItemsLength = items ? items.length : 12;
    this.createRenderer();
    this.createCamera();
    this.createScene();
    this.onResize();
  ...
  createMedias(items: { image: string; text: string }[] | undefined, bend: number = 1, textColor: string, borderRadius: number, font: string) {
    const defaultItems = [{ image: `https://picsum.photos/seed/1/800/600?grayscale`, text: 'Bridge' }];
  ...
  onTouchDown(e: MouseEvent | TouchEvent) {
    ...
    this.startTime = performance.now(); // Catat waktu mulai sentuh
  }
...
onTouchUp(e: MouseEvent | TouchEvent) {
    ...
    const movedEnough = Math.abs(x - this.startX) >= 15 || Math.abs(y - this.startY) >= 15;
...
export default function CircularGallery({
  items, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05, onClick
